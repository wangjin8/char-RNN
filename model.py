...
class CharRNN:
    def __init__(self, num_classes, num_seqs=64, num_steps=50,
        ...
        self.num_seqs = num_seqs  # 一个batch里面句子的数量
        self.num_steps = num_steps  # 句子的长度
    ...
    def build_inputs(self):
        with tf.name_scope('inputs'):
            # shape = (batch_size, num_steps) = (句子数量，句子长度)
            self.inputs = tf.placeholder(tf.int32, shape=(
                self.num_seqs, self.num_steps), name='inputs')

            # 输出shape=输入shape，内容是self.inputs每个字母对应的下一个字母
            ...
            if self.use_embedding is False:
                ...
                with tf.device("/cpu:0"):
                    # 先定义一个embedding变量，embedding才是我们的训练数据(字的总类别，每个字的向量)
    ...
    def build_lstm(self):
        ...
        with tf.name_scope('lstm'):
            ...
            # output_shape=(batch_size, time_steps, cell.output_size)=() time_steps步里所有输出，是个列表
            self.lstm_outputs, self.final_state = tf.nn.dynamic_rnn(cell, self.lstm_inputs, initial_state=self.initial_state)

            # 通过lstm_outputs得到概率
            seq_output = tf.concat(self.lstm_outputs, 1)                 #合并所有time_step得到输出, lstm_outputs只有一个，因此还是原shape
